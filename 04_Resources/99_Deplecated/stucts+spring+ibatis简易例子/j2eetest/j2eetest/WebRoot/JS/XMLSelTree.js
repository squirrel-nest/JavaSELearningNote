...
function TreeConfig(p_sXSLFile, p_oSrcDiv, p_bAutoGoFisrtLeaf, p_bChangeLeafImg)
{
	...
	this.bShowElseBranch	= true ;		// ??????????????????
											// true:   ?????????
											// false: ?????1??????????;
											// ????bug


	if (true==p_bChangeLeafImg)			// ???????????????

		this.bChangeLeafImg	= true;			// true:  ??;              
	else									// false: ???                
		this.bChangeLeafImg	= false;


	this.bDataAsync			= false ;		// ??????(???????????)
											// true:  ??; 
											// false: ?



	this.bCheckDataAsync	= true ;		// ??????(???????????)
											// true:  ??; 
											// false: ?
	...
	this.sCheckKey			= "," ;			// checkdata ???


	this.bCheckAsync		= true;			// ????????????
											// true:  ???; 
											// false: ??
	
	this.bCheckShowBranch	= false;		// ??????????????

											// true:  ??; 
											// false: ???

	if (true == p_bAutoGoFisrtLeaf)			// ???????????????'??'???  
		this.bAutoGoFisrtLeaf	= true;		// true:  ??;                
	else									// false: ???                
	...
	this.setGirdPageURL 	= setGirdPageURL;
	
	this.oXSLDoc = new ActiveXObject("Microsoft.XMLDOM") ;	// xsl???????xml??
	...
	function setGirdPageURL(pageURL){
		this.gridPageURL = pageURL;
	}
...
//	m_oConfig	= new TreeConfig(sXSLFile, p_oSrcDiv, p_bAutoGoFisrtLeaf, p_bChangeLeafImg) ;
	...
	m_oConfig	= new TreeConfig(p_sXSLPath, p_oSrcDiv, p_bAutoGoFisrtLeaf, p_bChangeLeafImg) ;
...
function GoFisrtLink(p_oSrcElement)
{
	...
	{
		oTag	= m_oConfig.oCurrentNode.getElementsByTagName("SPAN")[0];
		oLink	= GetChildElement(oTag, "link") ;
	}else{
		oTag	= p_oSrcElement.getElementsByTagName("SPAN")[0];
		oLink	= GetChildElement(oTag, "link") ;
...
function LoadNodeContent(p_oSrcElement)
{
    ...
    if (oLabel != null)
    {
        ...
        if (oLink != null)
        {
        	...
        	if(null != oLink.href && oLink.href != ""){
        	
        		switch(oLink.HrefType){
        			case "DataGrid" : 
        						sHref = m_oConfig.basePath + m_oConfig.gridPageURL + "?gridDataUrl=" + String(oLink.href);
        						break;	
        			case "Direct" : 
...
/*********************************************
**GoLink(sHref, sTargetFrame)
*********************************************/
function GoLink(sHref, sTargetFrame){

//	alert("Enter GoLink");
//	alert("sHref : " + sHref + "  ||  targetFrame : " + sTargetFrame);
	document.all(sTargetFrame).src = sHref;
...
function FoldElseBranch(p_oSrcElement)
{
	...
	if (m_oConfig.oCurrentNode != "moonpiazza")
	{
		...
		if ( !bIsAncestorNode )
		{
			...
			{
				alert("?????bug");
				FoldOffspringNode(p_oSrcElement.parentElement) ;			// ??????

				FoldNode(m_oConfig.oCurrentNode) ;							// ?? m_oConfig.oCurrentNode??

				FoldAncestorNode(p_oSrcElement, m_oConfig.oCurrentNode);		// ?? m_oConfig.oCurrentNode????		
			}
		}
...
function MouseClick(p_oSrcElement)
{
	...
	if (sSrcElementType == "label")
	{
		...
		if (m_oConfig.bAutoGoFisrtLeaf)
		{
			var oContainerElement	= GetChildElement(oTargetNode, "container") ;
			if (null != oContainerElement){
				var oLink = GoFisrtLink(oContainerElement.getElementsByTagName("DIV")[0]) ;
				if(null != oLink){
					//??????
				}
			}
