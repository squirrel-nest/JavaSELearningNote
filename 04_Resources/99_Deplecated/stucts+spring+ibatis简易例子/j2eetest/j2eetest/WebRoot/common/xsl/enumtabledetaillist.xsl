				  ...
				  <tr>
					  ...
					  <xsl:for-each select="/queryresult/list_fields/Head">
						  ...
						  <td align="left" nowrap="true">
							  <xsl:choose>
								  <xsl:when test="string(./NAMEEN)='I_ENUMTABLEDETAILID'">
								    <xsl:choose>
								      <xsl:when test="string(/queryresult/list/Result[position()=$currentnode]/I_OPTYPE)='0'">
								      <input type="checkbox" class="checkbox2">
										  <xsl:attribute name="name">
											  <xsl:value-of select="i_enumtabledetailid"/>
										  </xsl:attribute>
