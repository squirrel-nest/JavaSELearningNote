							...
							<xsl:choose>
								<xsl:when test="string(./YCHECK)='1'">
									<input type="checkbox" disabled="disabled">
										...
										<xsl:if test="/queryresult/list/Result[position()=$currentnode]/*[name()=$asname]='1'">
											<xsl:attribute name="checked"><xsl:value-of select="/queryresult/list/Result[position()=$currentnode]/*[name()=$asname]"/>&#160;></xsl:attribute>
										</xsl:if>
									</input>
								</xsl:when>
								<xsl:otherwise>
