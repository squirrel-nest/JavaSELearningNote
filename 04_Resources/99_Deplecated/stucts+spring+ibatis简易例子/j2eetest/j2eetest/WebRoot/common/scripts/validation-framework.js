 ...
 * $Id: validation-framework.js,v 1.1.1.1 2005/02/16 02:54:03 jerry Exp $
 */
...
// instead.
var ValidationRoot = "../common/scripts/";
...
ValidationFramework.validateDepend = function(field, depend) {
	...
	} else if (depend.getName() == "security") {
		return ValidateMethodFactory.validateSecurityID(field, depend.getParams());		
	} else {
...
ValidationFramework.getErrorString = function(field, depend) {
	...
	if (dep == "minlength" || dep == "maxlength" || dep == "date" ) {
		...
		retStr = retStr.replace("{1}", depend.getParams()[1]);
	}
/*
	} else if ( dep == "equalsfield") {
...
ValidateMethodFactory.validateSecurityID = function(field, params) {
	var obj = ValidationFramework.getWebFormFieldObj(field);
	if (obj == null) return false;
	if (obj.value == "") return true;
    var n;
    if(/^\d{15}$/.test(obj.value)) n = /^(\d\d)(\d\d)(\d\d)(\d\d)(\d\d)(\d\d)(\d{3})$/.exec(obj.value);
    if(/^\d{18}$/.test(obj.value)) n = /^(\d\d)(\d\d)(\d\d)(\d{4})(\d\d)(\d\d)(\d{4})$/.exec(obj.value);
    return n;
/*
	var formObj = document.getElementById(ValidationFramework._currentForm.getId());
	var eqField = formObj[params[0]];
	if (eqField != null && eqField.value==params[1]) {
	} else {
		return true;	
	}	
*/	
...
ValidationErrorString["zh-cn"]["security"]="{0}不是一个有效身份证号。";
...
ValidationErrorString["en-us"]["security"]="{0} is not an valid pid. ";
