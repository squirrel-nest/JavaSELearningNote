...
function WebFXLoadTree(sId,sText, sXmlSrc, sAction, sBehavior, sIcon, sOpenIcon) {
	// call super
	
	this.WebFXTree = WebFXTree;
	this.WebFXTree(sId,sText, sAction, sBehavior, sIcon, sOpenIcon);
...
function WebFXLoadTreeItem(sId,sText, sXmlSrc, sAction, eParent, sIcon, sOpenIcon) {
	// call super
	this.WebFXTreeItem = WebFXTreeItem;
	this.WebFXTreeItem(sId,sText, sAction, eParent, sIcon, sOpenIcon);
...
function _startLoadXmlTree(sSrc, jsNode) {
	...
	jsNode.loading = true;
	try{
	//sSrc="http://192.168.0.90:8080/xloadtree/tree1.xml";
	var xmlHttp = XmlHttp.create();
	xmlHttp.open("GET",sSrc,false);	// async
	xmlHttp.onreadystatechange = function () {
		if (xmlHttp.readyState == 4) {
            //window.alert("xmlHttp.responseText="+xmlHttp.responseText);
			//xmlHttp.responseXML="<?xml version='1.0'?><tree><tree text='Loads tree2.xml' src='tree2.xml'/></tree>";
...
function _xmlTreeToJsTree(oNode) {
	...
	var jsNode;
	if (src != null && src != "")
		jsNode = new WebFXLoadTreeItem(sId,text, src, action, parent, icon, openIcon);
	else
		jsNode = new WebFXTreeItem(sId,text, action, parent, icon, openIcon);
